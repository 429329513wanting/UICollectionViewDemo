...
- (void) addPageControl{
    ...
    // 设置显示标示颜色
    [_imageViewPageControl setPageIndicatorTintColor:[UIColor whiteColor]];
    [_imageViewPageControl setCurrentPageIndicatorTintColor:[UIColor blackColor]];
...
-(void) addImageviewsForMainScrollWithImages: (NSArray *) images{
    ...
    for ( int i = 0; i < _imageViewArray.count + 1; i ++) {
        ...
        //
        [tempImageView sd_setImageWithURL:[NSURL URLWithString:imageName] placeholderImage:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
